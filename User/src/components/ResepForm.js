        ...
        <Card className="bg-gray-100">
            ...
            <div className="relative flex flex-wrap justify-center bg-gray-100">
                {apiData.filter(val => {
                    ...
                    <Link key={recipes.id} class="cards w-72 bg-white shadow-md rounded-xl p-2 mx-3 my-5"  to={`/lihat-resep-makanan/${recipes.id}`}>
                        <div className="overflow-x-hidden rounded-xl relative">
                            <img src={`${BASE_URL}/recipe-detail-images/${recipes.recipe_image}`} alt={recipes.name} className="h-40 rounded-xl w-full object-cover"/>
                        </div>
